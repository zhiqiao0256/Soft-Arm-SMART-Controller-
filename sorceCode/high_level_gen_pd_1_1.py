...
class pc_client(object):
    ...
    def __init__(self):
        ...
        """ Format recording """
        self.array2setsRecord=np.array([0.]*40)#t pd1 pd2 pd3 + pm1 +pm2 +pm3 + positon +orintation
        ...
        self.smcNDOB_tracking=np.array([0.]*5)#p1_ub, u_total, u_eq, u_s, u_n, disturbance
        """ Thearding Setup """
        self.th1_flag=True
        self.th2_flag=True
    ...
    def th_pub_raspi_client_pd(self):
        try:
            ...
            if self.positionProfile_flag==3:
    ...
    def th_sub_pub_mocap(self):# thread config of read data from mocap and send packed msg to record file.
        try:
            while self.run_event.is_set() and self.th2_flag:
                ...
                self.smcNDOB_tracking=np.array([self.smcNDOB_p1,self.smcNDOB_u_total,self.smcNDOB_u_eq,self.smcNDOB_u_s,self.smcNDOB_u_n,self.smcNDOB_dist_est])
    ...
    def smcNDOBwithInputBoundAndSat(self):
        ...
        dtheta=self.x2_current
        # Update SMC 
        s=self.smcNDOB_lambda*self.x1_error_current+self.x1_dot_error
        # Update M,C,G
        Izz=self.m0*self.r0*self.r0
        if theta==0.:
            M=self.m0*(self.L/2)**2
