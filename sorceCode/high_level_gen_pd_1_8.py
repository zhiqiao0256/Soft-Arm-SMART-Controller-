        ...
        try:
            ...
            if self.flag_control_mode == 0: # SMC with input bound
                ...
                while self.t_old<=self.trailDuriation:
                    ...
                    print "time",np.round(self.t_old,1),"C?",self.flag_first_contact,"xdn:",np.round(np.rad2deg(self.xd_new),1),"x1d(deg)",np.round(np.rad2deg(self.xd1),1),"x1e(deg)",np.round(np.rad2deg(self.x1_error_old),1),"x1new_e(deg)",np.round(np.rad2deg(self.x1_new_error),1),'pd_ub',np.round(self.smcNDOB_p1,1),'pm',np.round(self.pd_pm_array[3],1),'d',np.round(self.smcNDOB_dist_est_tqr,4)
...
def main():
    try:
        ...
        p_client.flag_contact_mode=3 # 0: default smcndob 
                                     ...
                                     # 3: contact with fix position and stay and trying
        ...
        p_client.smcNDOB_th=0.4# N.m
        p_client.x1_error_max_value=np.deg2rad(0.8)
