...
class pc_client(object):
        ...
        try:
            ...
            if self.flag_control_mode == 0: # SMC with input bound
                ...
                while self.t_old<=self.trailDuriation:
                    ...
                    print "time",np.round(self.t_old,1),"C?",self.flag_first_contact,"xdn:",np.round(np.rad2deg(self.xd_new),1),"x1d(deg)",np.round(np.rad2deg(self.xd1),1),"x1e(deg)",np.round(np.rad2deg(self.x1_error_old),1),"x1new_e(deg)",np.round(np.rad2deg(self.x1_new_error),1),'pd_ub',np.round(self.smcNDOB_p1,1),'pm',np.round(self.pd_pm_array[3],1),'d',np.round(self.smcNDOB_dist_est_inner_tqr,2)
    ...
    def smcNDOBwithInputBoundAndSat(self):
        ...
        elif self.flag_contact_mode ==3: #fix to maximum position errors and stay with trying
            if np.absolute(self.smcNDOB_dist_est_tqr)>= self.smcNDOB_th:
                if np.absolute(self.x1_error_current) >= self.x1_error_max_value: # swith to compliant mode
                    if self.flag_first_contact ==0: # first contact
                        self.xd1_new=self.x1_old
                        self.smcNDOB_dist_est_inner=0.
                        self.smcNDOB_z_old_inner=0.
                    ...
                    if time()-self.timer_attampt <= self.timer_th:
                        ...
                        u_total = u_eq + u_s + u_n # trying to go back to xd_t
                        self.xd_new=xd1
                    elif time()-self.timer_attampt > self.timer_th: # trying to reach xd
                    ...
                    u_total = u_eq + u_s + u_n
                    self.xd_new=xd1
            elif np.absolute(self.smcNDOB_dist_est_tqr)< self.smcNDOB_th:
                self.xd1_new = self.positionProfileSelection()
                dxd1=self.positionSingleDerevativeSelection()
                ddxd1=self.positionDoubleDerevativeSelection()
                xd1=self.xd1_new
    ...
    def smcspoWithInputBoundAndSat(self):
        ...
        p1_MPa=(uAlpha/self.alpha0-(0.5*sphi+0.5*np.sqrt(3)*cphi)*pm2_MPa+sphi*pm3_MPa)/(0.5*sphi-0.5*np.sqrt(3)*cphi)
        p1_psi=p1_MPa*145.038
        print "pd",np.round(p1_psi,2),"pm",np.round(self.pd_pm_array[3],2),"x1Track",np.round(np.rad2deg(self.x1_current)),"x1d",np.round(np.rad2deg(self.xd1)),"x2_e",np.round(np.rad2deg(self.x2_e)),"per_est",np.round(self.smcspo_per_est,4)
        if p1_psi>=self.input_pressure_limit_psi:
            pd_array=np.array([self.input_pressure_limit_psi,1.0,1.0])
...
def main():
    try:
        ...
        p_client.positionProfile_flag=3 #0: sum of sine waves 
                                        #1: single sine wave, 
                                        #2: step
                                        #3: rampfromT0
        p_client.flag_control_mode=3# 0: baseline smc; 
                                    # 1: smc+ilc; 
                                    # 2: smc+spo;
                                    # 3: smc+ndob;
        p_client.flag_contact_mode=3 # 0: default smcndob 
                                     # 1: contact switch with fix pressure
                                     # 2: contact swithc with fix xd errors
                                     # 3: contact with fix x1 and multiple tries
        ...
        p_client.rampAmpAbs=np.radians(20) # x1(t0)-rampAmp
        p_client.rampFlatTime=10.0 # sec
        p_client.timer_th=5.0#sec
        # #### Test
