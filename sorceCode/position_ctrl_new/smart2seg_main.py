...
class pc_client(object):
    ...
    def th_pub_raspi_client_pd(self):
        try:
            ...
            # update tracking errors
                ...
                pd_1=self.smart(self.state_var_current[0],self.state_var_deri[0],vector_MCG_0,vector_xd_0,self.state_error_current[0],self.state_error_deri[0],time()-self.t_old)
    ...
    def th_sub_pub_mocap(self):# thread config of read data from mocap and send packed msg to record file.
        try:
            while self.run_event.is_set() and self.th2_flag:
                ...
                self.array3setsRecord=np.concatenate((self.pd_pm_array, self.array3setswithrotation, self.smc_tracking,self.smcNDOB_tracking), axis=None)
    ...
    def positionRampFromT0(self,t,x1_t0):
        ...
        if t <= self.rampAmpAbs/self.rampRateAbs:
            ...
            xd= x1_t0-self.rampAmpAbs +  self.rampRateAbs*(t-(self.rampAmpAbs/self.rampRateAbs+self.rampFlatTime))
            dxd=self.rampRateAbs
            ddxd=0.0
    ...
    def smart(self,state_current,state_var_deri,vector_MCG,vector_xd,state_error,state_error_deri,dt):
        theta=0.
        dtheta=0.
        s=0.
        M=vector_MCG[0]
        ...
        self.smcNDOB_dz_inner = -self.smcNDOB_kp_sig*(-self.smcNDOB_k_sig*s+b_x*(u_n+u_s)+self.smcNDOB_dist_est_inner)
        ...
        return pd
    def mode_switching(self,state_error,smcNDOB_dist_est_tqr):
