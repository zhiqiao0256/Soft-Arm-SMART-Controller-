...
class pc_client(object):
    ...
    def __init__(self):
        ...
        self.smcNDOB_kp_sig = 5.0#ksf in paper
        self.smcNDOB_kw=5.0
        ...
        self.smcNDOB_dist_est_inner_tqr_kw_s=0.
        ...
        try:
            ...
            if self.flag_control_mode == 0: # SMC with input bound
                ...
                while self.t_old<=self.trailDuriation:
                    ...
                    print "C?",self.flag_stage,"xdn:",np.round(np.rad2deg(self.xd_new),1),"xdt(deg)",np.round(np.rad2deg(self.xd1),1),"xt(deg)",np.round(np.rad2deg(self.x1_current),1),"xet(deg)",np.round(np.rad2deg(self.x1_new_error),1),'pm',np.round(self.pd_pm_array[3],1),'d',np.round(self.smcNDOB_dist_est_inner_tqr,2),'c1',np.round(self.smcNDOB_dist_est_inner_tqr_kw_s,2),'dz',np.sign(self.smcNDOB_dz_inner)*np.sign(self.smcNDOB_z_current_inner),'z',np.round(self.smcNDOB_z_current_inner,0)
    ...
    def smcNDOBwithInputBoundAndSat(self):
        ...
        if self.flag_contact_mode ==4: #N-C-R-C-R-N
            if self.flag_stage==0: # normal mode
                ...
                if np.absolute(self.x1_current-xd1) >= self.x1_error_max_value: #stay in normal mode OR switch to c mode
        ...
        # print pd_array
        #1229
        # print self.flag_stage,b_x*(u_n+u_s)+self.smcNDOB_dist_est_inner, self.smcNDOB_kp_sig*s
