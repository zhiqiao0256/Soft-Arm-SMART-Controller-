...
class pc_client(object):
        ...
        try:
            ...
            if self.flag_control_mode == 0: # SMC with input bound
                ...
                while self.t_old<=self.trailDuriation:
                    ...
                    print "time",np.round(self.t_old,1),"C?",self.flag_stage,"xdn:",np.round(np.rad2deg(self.xd_new),1),"xdt(deg)",np.round(np.rad2deg(self.xd1),1),"xt(deg)",np.round(np.rad2deg(self.x1_current),1),"xet(deg)",np.round(np.rad2deg(self.x1_error_current),1),'pm',np.round(self.pd_pm_array[3],1),'d',np.round(self.smcNDOB_dist_est_inner_tqr,2)
    ...
    def smcNDOBwithInputBoundAndSat(self):
        ...
        if self.flag_contact_mode==0: # normal smcndo
            ...
            self.smcNDOB_dz = -self.smcNDOB_kw*(b_x*(u_n+u_s)+self.smcNDOB_dist_est-self.smcNDOB_kp_sig*s)
            self.smcNDOB_z_current =self.smcNDOB_z_old + self.smcNDOB_dz*(self.t_new-self.t_old)
            self.smcNDOB_dist_est = self.smcNDOB_z_current+self.smcNDOB_kw*s
        ...
        #         self.xd_new=self.xd1 
            ...
            if self.flag_stage==0: # normal mode
                ...
                self.smcNDOB_z_old_inner=self.smcNDOB_z_current_inner
                u_total = u_eq + u_s + u_n
                if np.absolute(self.x1_error_current) >= self.x1_error_max_value: #stay in normal mode OR switch to c mode
                ...
                if np.absolute(self.smcNDOB_dist_est_inner_tqr)>= self.mode_C_to_R_th: #stay in compliant
                    dxd1=0.
                    ddxd1=0.
                    xd1=self.x1_current# update 1229
                    self.flag_stage =1
                    s =self.smcNDOB_lambda*(self.x1_current-xd1) +self.x2_current-dxd1
                    if np.absolute(s/self.smcNDOB_sat_bound) <=1:
                        sat_s= s/self.smcNDOB_sat_bound
