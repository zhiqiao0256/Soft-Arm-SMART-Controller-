...
class pc_client(object):
    ...
    def th_pub_raspi_client_pd(self):
        try:
            ...
            self.t0=time()
            self.t_old=time()
            # for index_trail in range(1,6,1):
            #     self.loop_timer=time()
            #     p1=np.random.randint(40, size=1)
            #     self.openloopStepPressureCtrl(np.array([p1,0.0,0.0]),30)
            f_f=0.001
            f_0=0.1
            t_total=30*3
            self.sum_of_sine2(f_f,f_0,t_total)            
            self.step_response(np.array([0.0,0.0,0.0]),5)
            self.th1_flag=False
            self.th2_flag=False
            exit()
    ...
    def sum_of_sine2(self,f_f,f_0,t_total):
        ...
        while time()<t_f:
            ...
            i=0
            for f_t in ftArray:
                p1=p1+40./numOfSines*np.sin(2*np.pi*f_t*t+phasArray[i])+20/numOfSines
                i=i+1
                # print p1
