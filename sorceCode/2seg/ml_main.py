...
class pc_client(object):
    ...
    def __init__(self):
        ...
        self.socket0.bind("tcp://10.203.48.122:4444")## PUB pd to Raspi Client
        ...
        """ Format recording """
        self.array3setsRecord=np.array([0.]*41)#t pd1 pd2 pd3 pd4+ pm1 +pm2 +pm3 pm4+ positon +orintation
    ...
    def th_pub_raspi_client_pd(self):
        try:
            ...
            self.x4_current=vector_phiTheta[0]
            self.t0=time()
            self.t_old=time()
    ...
    def openloopStepPressureCtrl(self,pd_array,step_time):
        while time()-self.loop_timer<= step_time and self.th1_flag:
            vector_phiTheta=np.array([0., 0.])
            phi=0.
            theta=0.
            dtheta=0.
            dphi=0.
            pm1_MPa=self.pd_pm_array[4]
            pm2_MPa=self.pd_pm_array[5]
            pm3_MPa=self.pd_pm_array[6]
            pm4_MPa=self.pd_pm_array[7]
            vector_phiTheta=self.getThetaPhiAndr0FromXYZ()
            phi=vector_phiTheta[0]
            theta=vector_phiTheta[1]
            phi1=vector_phiTheta[2]
            theta1=vector_phiTheta[3]
            #Update State variables x1,x2, x1 error,
            self.t_new=time()-self.t0
            self.x1_current=theta
            self.x2_current=phi
            self.x3_current=theta1
            self.x4_current=phi1
            ...
            sleep(0.005)
    def openloopRampPressureCtrl(self,pd_array):
            vector_phiTheta=np.array([0., 0.])
            phi=0.
            theta=0.
            dtheta=0.
            ...
            phi=vector_phiTheta[0]
            theta=vector_phiTheta[1]
            phi1=vector_phiTheta[0]
            theta1=vector_phiTheta[1]
            #Update State variables x1,x2, x1 error,
            self.t_new=time()-self.t0
            self.x1_current=theta
            self.x2_current=phi
