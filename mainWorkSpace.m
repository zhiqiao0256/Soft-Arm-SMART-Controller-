% Main function for stiffness ID use data 1025
%% Major chanages:
%%
clear all
close all
...
par_set.L=0.178;%actuator length
...
% Scatter Plot for workspace p1 vs. p23 vs theta
testData=[];
testData=par_set.trial1;
testData=func_getPhiThetaBfromXYZ(testData,par_set);
if par_set.plot_fwdKinematic == 1
...
xx=[];yy=[];zz=[];scatter_set=[];
index_0=1;index_1=1;index_2=1;index_3=1;index_4=1;
pe_MPa=testData.pd_MPa-testData.pm_MPa;
pe_psi=testData.pd_psi-testData.pm_psi;
for i = 1:length(testData.pd_MPa(:,1))
    if norm(pe_psi(i,2))<= 0.015*norm(testData.pd_psi(i,2))
        switch testData.pd_psi(i,3)
            case 0
                scatter_set{1}.xx(index_0,1)= testData.pm_psi(i,2);
                scatter_set{1}.yy(index_0,1)= 0.5*(testData.pm_psi(i,4)+testData.pm_psi(i,3));
                scatter_set{1}.zz(index_0,1)= testData.theta_deg(i,1);
                index_0=index_0+1;
            case 1.0
                scatter_set{2}.xx(index_1,1)= testData.pm_psi(i,2);
                scatter_set{2}.yy(index_1,1)= 0.5*(testData.pm_psi(i,4)+testData.pm_psi(i,3));
                scatter_set{2}.zz(index_1,1)= testData.theta_deg(i,1);
                index_1=index_1+1;
            case 1.5
                scatter_set{3}.xx(index_2,1)= testData.pm_psi(i,2);
                scatter_set{3}.yy(index_2,1)= 0.5*(testData.pm_psi(i,4)+testData.pm_psi(i,3));
                scatter_set{3}.zz(index_2,1)= testData.theta_deg(i,1);
                index_2=index_2+1;
            case 2.0
                scatter_set{4}.xx(index_3,1)= testData.pm_psi(i,2);
                scatter_set{4}.yy(index_3,1)= 0.5*(testData.pm_psi(i,4)+testData.pm_psi(i,3));
                scatter_set{4}.zz(index_3,1)= testData.theta_deg(i,1);
                index_3=index_3+1;
            case 2.5
                scatter_set{5}.xx(index_4,1)= testData.pm_psi(i,2);
                scatter_set{5}.yy(index_4,1)= 0.5*(testData.pm_psi(i,4)+testData.pm_psi(i,3));
                scatter_set{5}.zz(index_4,1)= testData.theta_deg(i,1);
                index_4=index_4+1;
        end
    end
end
figure
plot(pe_psi(:,3))
figure('Name','p1 vs p23 vs theta')
color_set=['r','b','k','g','c'];
sf=[];
for i=1:length(scatter_set)
    temp=scatter_set{i};
    sf(i)=scatter3(temp.xx,temp.yy,temp.zz,color_set(i));
    hold on
end
xlabel('pm1 psi')
ylabel('pm23 psi')
zlabel('\theta deg')
legend(sf,'0.0','1.0','1.5','2','2.5')
%
%% Grey-box system ID
testData=[];
testData=par_set.trial1;
...
avgModel.pi_grey=[1.2634,0.4897,0.8616];
testData2=par_set.trial1;
