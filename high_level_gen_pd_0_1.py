...
class pc_client(object):
	...
	def th_pub_raspi_client_pd(self):
			while self.run_event.is_set() and self.th1_flag:
				try:
					# self.step_response(np.array([0.0]*3),5)
					self.step_response(np.array([0,0,0,]),5)
					for i in range(5):
						# new_array=np.random.randint(23, size=(3))+2
						# self.ramp_response(np.array([3.0]*3),np.array([30.0,3.,3.,]),10)
						# self.step_response(np.array([30.0,3.,3.,]),5)
						# self.ramp_response(np.array([30.0,3.,3.,]),np.array([3.0]*3),10)
						# self.step_response(np.array([0.0]*3),10)
						# self.step_response(new_array,5)

						self.step_response(np.array([25,0.,0.,]),0.5)# p_high
						# self.step_response(np.array([1.0,25.,25.,]),20)#p_low
						self.step_response(np.array([0.0]*3),0.5)
